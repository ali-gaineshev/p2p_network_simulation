...
def cluster_graph(args):
    ...
    for _ in range(num_clusters):
        cluster_nodes = list(range(current_node, current_node + nodes_per_cluster))
    ...
    edges = list(G.edges())
    write_graph_to_txt(f"cluster_{num_clusters}_with_{nodes_per_cluster}_nodes_each.txt", total_nodes, edges)
    save_graph_as_image(G, f"cluster_{num_clusters}_with_{nodes_per_cluster}_nodes_each.png")
    save_graph_as_graphml(G, f"cluster_{num_clusters}_with_{nodes_per_cluster}_nodes_each.graphml")
...
def build_mega_graph(args):
    ...
    for spec in cluster_specs:
        ...
        for _ in range(num_clusters):
            clique_nodes = list(range(next_node_id, next_node_id + nodes_per_cluster))
